...
public class TweezeeReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context c, Intent i) {
		...
		Log.d("ULTIMATE SCHEDULER", "RECEIVED " + type + ":" + message + ". "
				+ Boolean.toString(date) + " : " + day);
		if (type.equals("tweet")) {
			...
			recipient = i.getStringExtra("recipient");
			username = Encryption.decryptString(i.getStringExtra("username"),
					Encryption.KEY);
			pass = Encryption.decryptString(i.getStringExtra("pass"),
					Encryption.KEY);
	...
	private void sendEmail() {
		if (day.split(",")[getcDay() - 1].equals("true") || date) {
			...
			try {
				sender.sendMail(subject, message, username, recipient);
			...
			if (!sendError) {
				...
				mHandler.post(new Runnable() {
					@Override
					public void run() {
						Toast.makeText(ctx, "Error while sending, " + message,
	...
	private void sendSMS() {
		if (day.split(",")[getcDay() - 1].equals("true") || date) {
			...
			try {
				...
				sms.sendTextMessage(recipient.replaceAll("-", ""), null,
						message, pi, null);
			} catch (Exception e) {
				sendError = true;
				e.printStackTrace();
	...
	private void sendTweet() {
		...
		if (day.split(",")[getcDay() - 1].equals("true") || date) {
			if (prefs.getBoolean("direct", false)) {
				try {
					t.updateStatus(mentions + " " + message + " " + mentions
							+ " " + getRandom());
					if (mentions.length() < 2 || mentions == null) {
						logSend("Twitter", getFormattedTime(), message, "--",
								true);
					} else {
						logSend("Twitter", getFormattedTime(), message,
								mentions, true);
			...
			if (!sendError) {
				...
				if (mentions.length() < 2 || mentions == null) {
					...
					logSend("Twitter", getFormattedTime(), message, mentions,
							false);
				}
				mHandler.post(new Runnable() {
					@Override
	...
	private void logSend(final String type, final String time, String message,
			final String to, final boolean success) {
		...
		try {
			fW = new FileWriter(Environment.getExternalStorageDirectory()
					+ "/t3hh4xx0r/ultimate_scheduler/log.txt", true);
		} catch (IOException e) {
			e.printStackTrace();
		}
		bW = new BufferedWriter(fW);
		try {
			bW.append("///" + type + "//" + time + "//" + message + "//" + to
