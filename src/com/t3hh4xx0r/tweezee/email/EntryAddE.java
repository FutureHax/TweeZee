...
public class EntryAddE extends SherlockActivity {
	...
	public void onCreate(Bundle icicle) {
		...
		passE = Encryption.encryptString(
				EmailActivity.accounts[p].getPassword(), Encryption.KEY);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == android.R.id.home) {
			...
			if (subjectET.getText().toString().length() != 0
					&& messageET.getText().toString().length() != 0
					&& recipientsMACTV.getText().toString().length() != 0) {
				if (time) {
					if (timeValue == null || timeValue.equals("")) {
						...
						if (date) {
							if (dateValue == null || dateValue.equals("")) {
								...
								if (!extras.getBoolean("editing", false)) {
									db.insertEEntry(
											...
											recipientsMACTV.getText()
													.toString(), timeValue,
											dateValue, Boolean
													.toString(startBoot), my_id);
									setupTimedEMail(this, usernE, passE,
											subjectET.getText().toString(),
											messageET.getText().toString(),
											myDaysBooleans, recipientsMACTV
													.getText().toString(),
											timeValue, false, my_id, dateValue);
									finish();
								} else {
									db.updateEEntry(usernE, subjectET.getText()
											.toString(), messageET.getText()
											.toString(), extras
							...
							if (!extras.getBoolean("editing", false)) {
								db.insertEEntry(usernE, subjectET.getText()
										.toString(), messageET.getText()
										.toString(), intervalET.getText()
										.toString(), myDaysBooleans,
										recipientsMACTV.getText().toString(),
										timeValue, "", Boolean
												.toString(startBoot), my_id);
								setupTimedEMail(this, usernE, passE, subjectET
										.getText().toString(), messageET
										.getText().toString(), myDaysBooleans,
										recipientsMACTV.getText().toString(),
								...
								db.updateEEntry(usernE, subjectET.getText()
										...
										.getText().toString(), myDaysBooleans,
										recipientsMACTV.getText().toString(),
										timeValue, Boolean.toString(startBoot),
										"");
								setupTimedEMail(this, usernE, passE, subjectET
										.getText().toString(), messageET
										.getText().toString(), myDaysBooleans,
						...
						if (date) {
							...
							if (!extras.getBoolean("editing", false)) {
								db.insertEEntry(
										...
										"", dateValue, Boolean
												.toString(startBoot), my_id);
								setupIntervalEmail(this, usernE, passE,
										subjectET.getText().toString(),
										messageET.getText().toString(),
										intervalET.getText().toString(),
										myDaysBooleans, recipientsMACTV
	...
	private void setupIntervalEmail(Context c, String username, String pass,
			...
			String recipients, boolean updating, int id, String date) {
	...
	private void setupTimedEMail(Context c, String username, String pass,
			...
			String timeValue, boolean updating, int id, String date) {
		Toast.makeText(c, "New email saved, " + message, Toast.LENGTH_LONG)
				.show();
		final DBAdapter db = new DBAdapter(this);
		db.open();
