...
public class EmailFragment extends SherlockListFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		final Vibrator vibe = (Vibrator) ctx
		...
		listView.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> arg0, View v,
				...
				try {
					while (c.moveToNext()) {
						if (Encryption.decryptString(
								...
								Encryption.KEY).equals(
								EmailActivity.accounts[pos].getName())) {
							mEntries.add(c.getString(c
									.getColumnIndex("message")));
							mRecipients.add(c.getString(c
									.getColumnIndex("send_to")));
							mUsers.add(Encryption.decryptString(
									c.getString(c.getColumnIndex("username")),
									Encryption.KEY));
						}
					}
				...
				BetterPopupWindowE dw = new BetterPopupWindowE.DemoPopupWindow(
						v, message, pos, recipient, user);
		...
		try {
			while (c.moveToNext()) {
				if (Encryption.decryptString(
						...
						Encryption.KEY).equals(
						EmailActivity.accounts[pos].getName())
						&& !entryArray.contains(c.getString(c
								.getColumnIndex("message")))) {
					entryArray.add(c.getString(c.getColumnIndex("message"))
							+ ":" + c.getString(c.getColumnIndex("send_to")));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
