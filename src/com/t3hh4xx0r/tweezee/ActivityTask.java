 ...
 * the Activity that the task will work with. Override <code>run()</code> and in
 * that method do whatever should be done in the background. You can also
 ...
 * running. It is crucial that <code>run()</code> must not attempt to access the
 * <code>ProgressDialog</code>, the activity UI, or any other part of the system
 * that is accessed through <code>Activity</code>, because the activity may be
 * destroyed at any time while the task is executing.
 * 
 * At the end of your <code>run()</code> method you will want to update the UI.
 * To do that you must call the <code>ActivityTask</code> instance's
 * <code>finish()</code> method, passing it an
 * <code>ActivityTask.Finisher</code> instance. The finisher's
 * <code>finish()</code> method will be called with an instance of your activity
 * as a parameter. That activity instance may be the original activity that
 * started the task, or it may be a new one if there were configuration changes
 * since the task was started.
 * 
 * <strong>Caution:</strong> It is crucially important that you only access the
 * activity through the parameter, and not via any implicit access that you
 * might have to the original activity if you define your
 * <code>ActivityTask</code> as an inner class inside your activity class. It
 * can be helpful to temporarily change your overridden methods to be static,
 * and check the compiler errors to see if you have inadvertently used any
 * implicit access.
 * 
 ...
 * using <code>getLastNonConfigurationInstance()</code> and storing the result
 * into the instance variable created in step 2. If it is not null then call
 * <code>newActivity(this)</code> on it.
 * 
 * 4) In your activity's <code>onDestroy()</code>, call
 * <code>activityDestroyed()</code> on your <code>ActivityTask</code> instance
 * if it is not null.
 * 
 ...
 * created in step 1 and assign it to the instance variable defined in step 2.
 * Creating the instance will immediately start the background task and display
 * the progress dialog if you defined one.
 */
public abstract class ActivityTask<A extends Activity> {

	protected ProgressDialog makeProgressDialog(A activity) {
		return null;
	}
