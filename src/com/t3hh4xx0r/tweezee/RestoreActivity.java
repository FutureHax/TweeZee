...
public class RestoreActivity extends SherlockListActivity {
	ListView lv1;
	String[] options = new String[] { "Twitter", "Email", "SMS" };
	String[] backups = new String[] { "twitter.txt", "email.txt", "sms.txt" };
	...
	public void onListItemClick(ListView lv, View v, final int p, long id) {
		...
		if (selectedBackup.exists()) {
			...
			builder.setMessage(
					"This will delete any currently saved entires. Continue?")
					.setCancelable(false)
		...
		try {
			...
			while ((l = bR.readLine()) != null) {
				if (l.startsWith("///")) {
					...
					db.insertEEntry(username, subject, message, send_wait,
							send_day, send_to, send_time, send_date,
							start_boot, Integer.parseInt(my_id));
		...
		try {
			...
			while ((l = bR.readLine()) != null) {
				if (l.startsWith("///")) {
					...
					db.insertTEntry(username, message, send_wait, send_day,
							mentions, send_time, start_boot,
							Integer.parseInt(my_id), send_date);
		...
		try {
			...
			while ((l = bR.readLine()) != null) {
				if (l.startsWith("///")) {
					...
					db.insertSEntry(message, send_wait, send_day, send_to,
							send_time, start_boot, Integer.parseInt(my_id),
							send_date);
