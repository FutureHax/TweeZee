	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		listView.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> a, View v,
				...
				try {
					while (c.moveToNext()) {
						if (c.getString(0).equals(
								TwitterActivity.users[pos].getName())) {
				...
				BetterPopupWindow dw = new BetterPopupWindow.DemoPopupWindow(v,
	...
	void populateList() {
		...
		try {
			while (c.moveToNext()) {
				...
				if (c.getString(0).equals(TwitterActivity.users[pos].getName())
						&& !entryArray.contains(c.getString(1) + " "
								+ c.getString(c.getColumnIndex("mentions")))) {
