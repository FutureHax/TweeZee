...
public class EntryAdd extends SherlockActivity {
	...
	public void onCreate(Bundle icicle) {
		...
		p = extras.getInt("pos");
		startBoot = extras.containsKey("boot") ? Boolean.parseBoolean(extras
				.getString("boot")) : false;
		...
		if (extras.getBoolean("editing", false)) {
			incomingT = extras.getString("mentions").replaceAll("@", "")
					.length();
		}
		myCount.setText("0");
		...
		if (extras.getBoolean("editing", false)) {
			...
			if (extras.containsKey("time")
					&& extras.getString("time").length() > 1) {
	...
	private void getFollowing() {
		...
		try {
			...
			String[] screenNames = (String[]) names.toArray(new String[names
					.size()]);

			ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
					android.R.layout.simple_dropdown_item_1line, screenNames);
	...
	public Drawable setProfilePic(String name) {
		...
		File file = new File(Environment.getExternalStorageDirectory()
				+ "/t3hh4xx0r/ultimate_scheduler/profileimages/twitter_" + name
				+ "_image_small.jpg");
		File dir = new File(Environment.getExternalStorageDirectory()
				+ "/t3hh4xx0r/ultimate_scheduler/profileimages/");
		Drawable d;
		if (!file.exists()) {
			if (!dir.exists()) {
				dir.mkdirs();
			...
			try {
				...
				URL src = new URL(u.getBiggerProfileImageURL());

				Bitmap bm = BitmapFactory.decodeStream(src.openConnection()
						.getInputStream());
		...
		if (item.getItemId() == android.R.id.home) {
			...
			if (totalC < 140) {
					...
					if (time) {
						if (timeValue == null || timeValue.equals("")) {
							...
							if (date) {
								if (dateValue == null || dateValue.equals("")) {
									Toast.makeText(
											this,
											"Do not leave any fields blank. dateValue",
											Toast.LENGTH_LONG).show();
								} else {
									if (!extras.getBoolean("editing", false)) {
										db.insertTEntry(
												TwitterActivity.users[p]
														.getName(),
												et1.getText().toString(),
												et3.getText().toString(),
												"false,false,false,false,false,false,false,",
												users, timeValue, Boolean
														.toString(startBoot),
												my_id, dateValue);
										setupTimedTweet(this, usern, et1
												.getText().toString(),
												myDaysBooleans, users,
								...
								if (!extras.getBoolean("editing", false)) {
									db.insertTEntry(
											TwitterActivity.users[p].getName(),
											et1.getText().toString(),
											et3.getText().toString(),
											"false,false,false,false,false,false,false,",
											users, timeValue, Boolean
													.toString(startBoot),
											my_id, "");
									setupTimedTweet(this, usern, et1.getText()
											.toString(), myDaysBooleans, users,
											timeValue, false, my_id, null);
									finish();
								} else {
									db.updateTEntry(TwitterActivity.users[p]
											.getName(), et1.getText()
											.toString(), users, extras
											.getString("message"), et3
											.getText().toString(),
											myDaysBooleans, timeValue, Boolean
													.toString(startBoot), "");
									setupTimedTweet(this, usern, et1.getText()
											.toString(), myDaysBooleans, users,
						...
						if (et3.getText().toString().length() == 0) {
							...
							if (date) {
								...
								if (!extras.getBoolean("editing", false)) {
									db.insertTEntry(TwitterActivity.users[p]
											.getName(), et1.getText()
											.toString(), et3.getText()
											.toString(), myDaysBooleans, users,
											"", Boolean.toString(startBoot),
											my_id, dateValue);
									setupIntervalTweet(this, usern, et1
											.getText().toString(), et3
											.getText().toString(),
											myDaysBooleans, users, false,
											my_id, dateValue);
									finish();
								} else {
									db.updateTEntry(TwitterActivity.users[p]
											...
											.getString("message"), et3
											.getText().toString(),
											myDaysBooleans, "", Boolean
													.toString(startBoot),
											dateValue);
									setupIntervalTweet(this, usern, et1
											.getText().toString(), et3
											.getText().toString(),
											myDaysBooleans, users, true, 420,
											dateValue);
									finish();
								}
							} else {
				...
				new AlertDialog.Builder(this)
						.setTitle("Yikes!")
						.setMessage(
								"The Twitter character limit is 140.\nYou are "
										+ Integer.toString(totalC - 140)
										+ " over the limit.")
	...
	private void setupIntervalTweet(Context c, String username, String message,
			String wait, String day, String mentions, boolean updating, int id,
			String date) {
		Toast.makeText(c, "New tweet saved, " + message, Toast.LENGTH_LONG)
				.show();
		final DBAdapter db = new DBAdapter(this);
		db.open();
		Cursor cu = null;
	...
	private void setupTimedTweet(Context c, String username, String message,
			String day, String mentions, String timeValue, boolean updating,
			int id, String date) {
		Toast.makeText(c, "New tweet saved, " + message, Toast.LENGTH_LONG)
				.show();
		final DBAdapter db = new DBAdapter(this);
