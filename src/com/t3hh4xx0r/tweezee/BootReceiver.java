	    	...
	    	while (c.moveToNext()) {
	       		...
	       		if (c.getString(c.getColumnIndex("start_boot")).equals("true")) {	       			
	       			if (c.getString(c.getColumnIndex("send_time")).length()>2) {
	       				...
	       				setupIntervalSMS(ctx, message, wait, day, recipient, getSID(recipient, message, ctx));	       				
	       			}
       				db.updateActiveS(Integer.toString(getSID(recipient, message, ctx)), true);
	       		} else {
       				db.updateActiveS(Integer.toString(getSID(recipient, message, ctx)), false);
	       		}
	       	}
	    } catch (Exception e) {   	
       		 e.printStackTrace();
	...
	private void setupEmails(Context ctx) {
	    ...
	    try {
	    	while (c.moveToNext()) {
	       		...
	       		String pass = Encryption.decryptString(c.getString(c.getColumnIndex("password")), Encryption.KEY);
	       		String subject = c.getString(c.getColumnIndex("subject"));
	       		if (c.getString(c.getColumnIndex("start_boot")).equals("true")) {	       			
	       			if (c.getString(c.getColumnIndex("send_time")).length()>2) {
	       				setupTimedEmail(ctx, user, pass, subject, message, day, recipient, time, getEID(user, message, ctx));
	       			} else {
	       				setupIntervalEmail(ctx, user, pass, subject, message, wait, day, recipient, getEID(user, message, ctx));	       				
	       			}
